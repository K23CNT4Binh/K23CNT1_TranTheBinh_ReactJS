...
export default function TtbCreateUser() {
    const ttbUser = {
        id:0,
        'ttb_name':'',
        'ttb_email':'',
   ...
   const ttbApi = "https://67e0ab247635238f9aae06d0.mockapi.io/ttb-k23cnt1-tranthebinh-2310900012/ttb_users"
    ...
    const ttbHandleSubmit = (ev)=>{
        ev.preventDefault();
        console.log(ttb_user);
        axios
            .post(ttbApi, ttb_user)
            .then(()=>{
                navigator("/ttb-list-user")
            })
            .catch((error) => console.log("Loi khi them user"));
    }
  return (
    <div>
      <h2>Thêm mới User</h2>
